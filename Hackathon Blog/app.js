    ...
    parent.innerHTML += ` <div class="col-lg-4 col-md-6 col-sm-12 my-2">
      <div class="card">
        <h5 class="card-header">
          <img src="${value.imageUrl}" width="100%" height="200px" alt="" />
        </h5>
        <div class="card-body">
          <h5 class="card-title">${value.title}</h5>
          <p class="card-text">${value.content}</p>
          <button onclick="deleteblog('${value.id}')" 
           style="
            ...
            >Delete Post</button>
          <button onclick="editblog(this, '${value.id}')" 
           style="
                background-color: black;
                border: 2px solid white;
                color: white;
...
const deleteblog = async (blogId) =>{
  ...
  alert("blog is deleted from the dashboard")
  // renderBlogUI(await fetchAllBlogs());
}
    

const editblog = (button, blogId) => {
  const blogCard = button.closest('.card'); // Find the blog card
  const blogTitle = blogCard.querySelector('.card-title').innerText;
  const blogContent = blogCard.querySelector('.card-text').innerText;
...
// Function to update blog in Firebase and UI
const updateBlog = async () => {
  ...
  // Update blog on the UI
  const blogCard = document.querySelector(`button[onclick="editblog(this, '${blogId}')"]`).closest('.card');
  blogCard.querySelector('.card-title').innerText = updatedTitle;
  blogCard.querySelector('.card-text').innerText = updatedContent;
